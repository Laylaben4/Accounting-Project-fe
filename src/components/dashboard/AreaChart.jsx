...
export const AreaChart = memo(function AreaChart({ data, label }) {
  ...
  const area = `${line} L${points.at(-1).x},${PADDING.top + innerH} L${points[0].x},${PADDING.top + innerH} Z`;
  ...
  return (
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-full w-full" role="img" aria-label={label}>
      ...
      {points.map((p) => (
        <text key={p.month} x={p.x} y={HEIGHT - 4} textAnchor="middle" className="fill-slate-400 text-[11px]">
          {p.month}
        </text>
      ))}
    </svg>
